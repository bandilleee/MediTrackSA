...
const ShaderCanvas = () => {
  ...
  const [backgroundColor, setBackgroundColor] = useState([0.05, 0.05, 0.1]); // Dark blue-black base

  useEffect(() => {
    const root = document.documentElement;
    const updateColor = () => {
      const isDark = root.classList.contains('dark');
      setBackgroundColor(isDark ? [0, 0, 0] : [0.95, 0.95, 0.95]);
    };
    updateColor();
  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      void main(){
        ...
        vec3 foregroundColor=vec3(0.3+v.x*0.3,0.4,0.7-v.y*0.3); // Purple-blue accent
...
export const PricingCard = ({
  ...
  const cardClasses = `
    ...
    from-gray-900/30 to-black/10 border border-gray-700
    ${isPopular ? 'scale-105 relative ring-2 ring-blue-400/20 shadow-2xl' : ''}
  `;
  const buttonClasses = `
    mt-auto w-full py-2.5 rounded-xl font-semibold text-[14px] transition font-sans
    ${buttonVariant === 'primary' 
      ? 'bg-blue-500 hover:bg-blue-400 text-white' 
      : 'bg-black/10 hover:bg-gray-800 text-white border border-gray-700'
    }
  `;
  ...
  return (
    <div className={cardClasses.trim()}>
      ...
      <div className="mb-3">
        ...
        <p className="text-[16px] text-gray-300 mt-1 font-sans">{description}</p>
      </div>
      <div className="my-6 flex items-baseline gap-2">
        <span className="text-[48px] font-extralight text-white font-display">R{price}</span>
        <span className="text-[14px] text-gray-400 font-sans">/mo</span>
      </div>
      <div className="card-divider w-full mb-5 h-px bg-gradient-to-r from-transparent via-blue-400/30 to-transparent"></div>
      <ul className="flex flex-col gap-2 text-[14px] text-gray-300 mb-6 font-sans">
        {features.map((feature, index) => (
          <li key={index} className="flex items-center gap-2">
            <CheckIcon className="text-blue-400 w-4 h-4" /> {feature}
          </li>
        ))}
      </ul>
...
export const ModernPricingPage = ({
  ...
  return (
    <div className="bg-gradient-to-b from-gray-900 to-black text-white min-h-screen w-full overflow-x-hidden">
      {showAnimatedBackground && <ShaderCanvas />}
      <main className="relative w-full min-h-screen flex flex-col items-center justify-center px-4 py-8 mt-15">
        <div className="w-full max-w-5xl mx-auto text-center mb-14">
          <h3 className="text-[48px] md:text-[64px] font-extralight leading-tight tracking-[-0.03em] text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-purple-400 to-cyan-400 font-display">
            {title}
          </h3>
          <p className="mt-3 text-[16px] md:text-[20px] text-gray-300 max-w-2xl mx-auto font-sans">
            {subtitle}
          </p>
        </div>
